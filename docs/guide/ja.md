...
3. エージェントはモデルのファイルを丸ごと読まず、1 画面の要約だけを見ます。パイロットのフィールドのうち、あなたのモデルにないものを選んで置き換えます。

データ接続（SQL Server、ファイル、ODBC など）はモデルと一緒にコピーされ、あなたの PC の中だけにあります。
**ODBC はまだ十分に検証できていません。** ODBC を使う場合は、既存のレポートを 1 つ PBIP で保存し、先にエージェントにデザインと接続方法を読ませてください。環境に合った結果になります。
