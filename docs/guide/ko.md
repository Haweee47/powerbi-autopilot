...
3. 에이전트는 모델 파일을 통째로 읽지 않고 한 화면 요약만 봅니다. 파일럿 필드 중 내 모델에 없는 것을 골라 바꿉니다.

데이터 연결(SQL Server, 파일, ODBC 등)은 모델과 함께 복사되고 내 PC에만 있습니다.
**ODBC는 아직 충분히 검증하지 못했습니다.** ODBC를 쓴다면 기존 리포트 하나를 PBIP로 저장해 에이전트에게 디자인과 연결 방식을 먼저 학습시키세요. 내 환경에 더 잘 맞는 결과가 나옵니다.
