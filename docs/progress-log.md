...
**틀린 것: Desktop 자동 업데이트.** 첫 사용자 흐름을 캡처하다가 대시보드가 09-13 캡처와 다르게 보이는 걸 발견했다.
...
- 원인을 하나씩 지웠다. 테마(커밋된 페이퍼 캡처는 정상) → 언어(영어·일본어 빌드 차이는 "Total" 한 단어) → 화면 크기(창을 접어 110%로 키워도 같음)
  → 적용 대기 상태(변경 내용 적용 후에도 같음) → 페이지 다시 그리기(같음). 마지막으로 생성된 파일을 커밋된 파일럿과 비교하니 **데이터 경로 한 줄 말고는 같았다**.
- 남은 변수는 Desktop 버전이었다. Microsoft Store판이 2.157.879.0에서 **2.157.1354.0**으로 자동 업데이트돼 있었다.
- 배운 점: 검증기 통과와 한 번의 캡처로는 부족하다. Desktop 버전이 바뀌면 파일럿을 다시 캡처해야 한다.
  CHANGELOG의 알려진 문제에 적었고, 고친 뒤에 v0.1.0을 릴리스하기로 했다.
