...
| 공개 도구와 비교 | 같은 모델 위에 같은 페이지를 세 방식으로: 이 저장소의 생성기, Microsoft `powerbi-report-cli`(비주얼 8개에 수기 JSON 13.7KB), data-goblin `pbir` CLI. 지침 비용은 ~1.7k 토큰 대 ~4.4k·~32.6k이고, 공개 도구 두 경로 모두 검증은 통과했지만 화면에 결함이 남았다 ([예제 01](examples/01-microsoft-skill/README.md), [예제 02](examples/02-data-goblin/README.md)) |
