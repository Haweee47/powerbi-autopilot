...
Microsoft와 data-goblin 모두 Power BI용 Claude Code 플러그인을 낸다. **같은 모델 위에 같은 페이지**를 각각의 방식으로 끝까지 만들어
보고, 주장 대신 재 봤다 ([예제 01](examples/01-microsoft-skill/README.md) · [예제 02](examples/02-data-goblin/README.md)):

| | 에이전트가 읽는 지침 | 에이전트가 쓰는 양 | 결과 |
|---|---|---|---|
| **이 저장소** | **~1.7k 토큰** | 모델 대응표 5.1KB | 4페이지 · 비주얼 55개 · 테마 적용 |
| Microsoft `powerbi-report-cli` | 참조 파일 ~89KB (~4.4k + 14.7k 토큰) | `visual.json` 전량 — 8개에 13.7KB | 1페이지, 검증은 통과하나 화면에 결함 |
| data-goblin `reports` | 스킬 5종 ~32.6k 토큰 | `pbir` 명령 ~600바이트 | 7개 중 2개가 렌더링되지 않음 |

Microsoft CLI에는 생성 명령이 없어 비주얼 JSON을 손으로 쓴다 — 그 스킬 문서 자체가 "반복이 제약이면 결정론적 생성기를 쓰라"고 권한다.
data-goblin의 `pbir`는 진짜 생성기지만, 리포트 생성에 Fabric 로그인과 발행된 모델이 필요해 로컬만으로는 시작할 수 없다.

**공개 도구 두 경로 모두 각자의 검증기를 통과하고도 화면은 틀렸다.** 그래서 여기서는 모든 페이지를 Power BI Desktop에서 열어 캡처한
뒤에야 완료라고 부른다. 그 기록의 채점 점수에는 편향을 함께 적어 두었다 — 채점표를 내가 썼기 때문이다.
...
![테마 10종](docs/share/media/themes.png)

테마는 색 팔레트 · 카드 모양 · 글꼴 세트의 조합이다. 토큰 파일 하나에서 생성하고 모두 Power BI 공식 테마 스키마(2.157)를 통과한다.
리포트를 어디서 볼지로 고른다.
...
| | **유니버설** | soft | 색각 이상·프로젝터·인쇄에서도 구분되는 Okabe-Ito 색 |
| | **카본** | flat, 어두운 레일, DIN 숫자 | 계열이 많은 운영 화면 |
| 쇼케이스 | **브로드시트** | flat, 따뜻한 종이색, 세리프 제목 | 읽는 자료(이사회 보고·월간 리뷰) |

계열 색은 모두 색각 이상 검사를 통과한다(적·녹·청색맹에서도 이웃 계열이 구분된다). 두 번째 색은 작년 값을 위해 일부러 회색이다.
글자도 검사한다. 모든 글자를 바로 뒤 배경색과 대비 계산해 WCAG 4.5:1 미만이면 빌드를 실패시킨다.

**글꼴**이 세 번째 축이다. Power BI가 직접 들고 있는 글꼴만 쓴다. 그 밖의 글꼴은 보는 사람 PC에 없으면 조용히 대체되기 때문이다.
`segoe`는 기본, `din`은 Power BI 표지판용 글꼴(숫자가 계기판처럼 보인다), `editorial`은 Georgia 제목 + Corbel 본문. 세 세트 모두 같은 CJK 대체 글꼴로 이어진다.

**배치 두 가지:** 모든 파일럿 페이지를 왼쪽 레일형(기본) 또는 위쪽 막대에 페이지·필터를 두고 본문을 전체 폭으로 쓰는 상단 메뉴형(`--frame top`)으로 만들 수 있다.

**회사 브랜드 색:** `python tools/brand_theme.py --id acme --accent "#0F62FE" --base navy`로 색 하나에서 열한 번째 테마를 만든다. 파랑·청록·보라 계열은 데이터 색까지, 빨강·주황·노랑·초록 계열은 레일과 선택 표시에만 쓴다. 리포트에서 빨강은 이미 "목표 미달"이기 때문이다([안내서](docs/guide/ko.md)).
...
| 내 데이터, 모델은 아직 없음 | `new_model.py`가 **CSV 폴더·엑셀 파일·ODBC로 붙는 DB**에서 시맨틱 모델을 만든다. 타입은 원본에서 읽고, 키 관계와 달력은 추론하고, 기본 측정값까지 쓴다. 같은 파일럿과 같은 모델 대응표가 세 경로 모두에서 그대로 돈다 ([예제 07](examples/07-own-data/README.md)) |
...
├── design-system/     원칙·채점표·연구 정리, 토큰 → 테마 10종(색 × 카드 모양 × 글꼴), 레이아웃 템플릿, 언어 등록부, HTML 시안
├── tools/             생성기(명세 → PBIR), 새 리포트 시작, 테마·레이아웃 빌드, 디자인 검사, 공유 이미지, 토큰 측정
...
- [x] 용도별 파일럿 4종 · 풀필먼트 운영 파일럿 · 테마 10종 · 배치 2종 · 다국어
- [ ] 표 안의 미니 추이선(SVG 측정값)
