...
CSV_TABLES = {
    ...
    "Labor": ("Labor.csv", [("Date", "dateTime", ["formatString: yyyy-mm-dd", "isHidden"]), ("Hour", "int64", ["isHidden"]),
                            ...
                            ("Travel Distance", "double", ["isHidden"])]),
    "Orders": ("Orders.csv", [("Date", "dateTime", ["formatString: yyyy-mm-dd", "isHidden"]), ("Cutoff", "string", []),
                              ("Carrier", "string", []), ("Order Type", "string", ["isHidden"]), ("Orders", "int64", ["isHidden"]), ("Orders Due", "int64", ["isHidden"]),
...
CALC_TABLES = {
    ...
    "Order Types": ("DATATABLE ( \"Order Type\", STRING, \"Order Type Order\", INTEGER, { { \"Single unit\", 1 }, { \"Multi unit\", 2 }, { \"Bulk\", 3 } } )",
                    [("Order Type", ["summarizeBy: none", "sortByColumn: 'Order Type Order'"]), ("Order Type Order", ["isHidden", "summarizeBy: none"])]),
...
MEASURES = [
    ...
    ("2. Productivity", "Travel Distance", "SUM ( Labor[Travel Distance] )", "#,0", "Metres walked on the floor (picking, putaway, counting)."),
    ("2. Productivity", "DPU",
     "DIVIDE ( [Travel Distance], CALCULATE ( [Units], KEEPFILTERS ( Labor[Travel Distance] > 0 ) ) )", "0.0",
     "Distance per unit: metres walked for each unit handled, over the work that walks (picking, putaway, counting). "
     "Zone layout and the order mix move it."),
    ("2. Productivity", "Pick DPU", "CALCULATE ( [DPU], Processes[Process ID] = \"PCK\" )", "0.0", "Metres walked per picked unit."),
    ("2. Productivity", "DPU Target", "13.5", "0.0", "Plan: 13.5 metres per picked unit."),
    ...
    ("3. Outbound", "Order Mix", "DIVIDE ( [Orders Shipped], CALCULATE ( [Orders Shipped], ALLSELECTED ( 'Order Types' ) ) )", "0.0%",
     "Share of the orders in view that are of this type."),
    ("3. Outbound", "Single-unit Share", "DIVIDE ( CALCULATE ( [Orders Shipped], 'Order Types'[Order Type] = \"Single unit\" ), CALCULATE ( [Orders Shipped], REMOVEFILTERS ( 'Order Types' ) ) )", "0%", "Orders with one unit: they walk the most per unit."),
...
RELATIONSHIPS = [
    ...
    ("Counts_Zones", "Counts.'Zone ID'", "Zones.'Zone ID'"), ("Orders_OrderTypes", "Orders.'Order Type'", "'Order Types'.'Order Type'"),
]
