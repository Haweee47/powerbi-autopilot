...
- A domain pilot for fulfillment-center operations (`--purpose fulfillment`), with its own model (12 tables, 51 measures) and seeded
  ...
  adds DPU (metres walked per unit) with order type as a filter: one dot per day, share of single-unit orders against pick DPU.
  Concepts and formulas: `design-system/domains/fulfillment.md`
...
- A second layout for every page: `--frame top` puts the report name, page tabs and slicers in a bar across the top and gives
  the body the full width (columns 72 → 88 px). Built from the same layout templates, so specs don't change. The headline keeps
  its rail height, because 24 px clipped its descenders; the title sits directly under the bar
